...
mdatatemp(float* __restrict__ tx, float* __restrict__ ty, float* __restrict__ tz,
          ...
          for (int ii = (blockIdx.y * blockDim.y + threadIdx.y) * blockDim.x * gridDim.x + (blockIdx.x * blockDim.x + threadIdx.x);
            ...
            sum_x[ii] += brmsx * delta_time * (dst_sin_x[ii] * cos(ctime * wc_val) - dst_cos_x[ii] * sin(ctime * wc_val));
            sum_y[ii] += brmsy * delta_time * (dst_sin_y[ii] * cos(ctime * wc_val) - dst_cos_y[ii] * sin(ctime * wc_val));
            sum_z[ii] += brmsz * delta_time * (dst_sin_z[ii] * cos(ctime * wc_val) - dst_cos_z[ii] * sin(ctime * wc_val));
 ...
 // float3 pxm = cross(p, m);
          ...
          float spin_constant = 1;//2 / HBAR; // debemos dividir entre gamma0 nuestro nuevo termino? parece que si

          // float result =  sum_x[i] +  sum_y[i] +  sum_z[i]
          // float3 final = result * spin_constant * mxBrms;

           float3 torque = spin_constant * result_sum * mxBrms;
