...
func AddCavity(dst, m, brms, scn *data.Slice, wc, kappa MSlice, x0, p0, msatCell, deltah, ctime float64, mem *[2]float64, mesh *data.Mesh, customKernel bool) {
	...
	if customKernel {
		...
		(*mem)[0] += math.Exp(kappa_temp*ctime) * math.Sin(wc_temp*ctime) * float64(brms_m) * deltah
		(*mem)[1] += math.Exp(kappa_temp*ctime) * math.Cos(wc_temp*ctime) * float64(brms_m) * deltah
